...
| Phase A.3 후 | + **구글 OAuth** (메인). 매직링크는 폴백 유지 |
| 메인 URL | 곧바로 LoginScreen — 구글 클릭 1회 또는 매직링크 |

- **2026-05-19 grill-me 갱신**: 이메일+비밀번호는 **제외**. 초대제 게이팅 하에서 매직링크(전원)+구글이 전원을 커버 — 비밀번호는 재설정 플로우 등 유지보수 비용 대비 가치 낮음. Phase C(공개 전환) 때 재검토. 상세: `PLAN-037`.
- **가입 게이팅**: Phase A·B 는 초대제 (Supabase 자율가입 OFF + 관리자 초대). Phase C 에 자율 개방.
