...
#### Step 3.5: 정합성 검증 구현
- [x] 검증용 프롬프트 생성 함수 (Step 3.2에서 미리 구현 — `buildVerifyPrompt`)
- [x] 개별/전체 검증 버튼 (MainTable 행/툴바)
- [x] 검증 결과 표시 (일치/불일치/확인불가) — `VerificationCard.jsx`
- [x] 수정 제안 수용/리젝 — `applyVerifyUpdate`로 불일치 필드만 마스터에 반영
- **확인**: IHTC "검증" 클릭 → 주기(4년), 지역(세계) 등이 일치/불일치로 표시되는가?

#### Step 3.6: 실전 검증 (AI 업데이트 품질 확인) — **완료 (2026-04-16)**

> Phase A로 rate limit이 해소되어 재개. 개별 업데이트 4건으로 품질·UX 동시 검증.

- [x] 공식사이트가 이미 열린 학회 3~5건을 선별 (IIR GL / IHTC / ICCFD / ICBCHT — 4건)
- [x] 해당 학회들에 대해 개별 업데이트 실행
- [x] AI가 찾아온 결과가 공식사이트 정보와 일치하는지 수동 대조
- [x] 결과를 docs/PROMPT_LOG.md에 기록 (4건 pass, 리스팅 사이트 선호 이슈 관찰)
- [ ] ~~필요시 프롬프트 수정 → 재테스트 → 기록 (반복)~~ — v2 프롬프트 과제로 MVP 후 이월 (`PROMPT_STRATEGY.md §1`)
- [ ] ~~전체 업데이트도 1회 실행하여 전체 흐름 확인~~ — 스킵, eval 17건 + 실전 4건으로 충분. 자연 사용 중 관찰로 대체
- **확인**: 4건 모두 정확한 upcoming 정보 획득, UX 흐름(업데이트 → 수용 → 테이블 반영 → GitHub 저장) 정상 동작 ✅

> ✅ Phase 3 완료: AI 업데이트와 정합성 검증 기능 동작, 실전 4건으로 품질 확인. v2 프롬프트 개선은 MVP 후 이월.
...
#### Step 4.1: 실전 검증 결과 반영 — **완료 처리 (2026-04-16, MVP 후 QA 이관)**

> Step 3.6 실전 4건에서 UI/UX 차원의 구체적 문제가 관찰되지 않음. 선제적 수정보다는 MVP 후 자연 사용 기반 QA로 수집·일괄 반영이 효율적이라 판단.
> 발견되는 이슈는 MVP 후 `docs/CHANGELOG.md`에 기록하고 일반 QA 흐름으로 처리.

- [x] ~~Step 3.6에서 발견된 UI/UX 문제 수정~~ — 발견된 문제 없음
- [ ] ~~에러 메시지 사용자 친화적으로 개선~~ — MVP 후 QA 이관
- [ ] ~~로딩 상태 표시 개선~~ — MVP 후 QA 이관
- **확인**: 실전 4건에서 UX 흐름(업데이트/검증/수용/리젝/GitHub 저장) 정상 동작 확인됨 ✅

#### Step 4.2: UI 폴리싱 — 코드 수정 완료 (2026-04-16), 실측 대기

> 스캔 결과 전반적으로 일관적. 튀는 포인트 소수만 정리.
> 상세 결정은 `docs/DESIGN.md` 2026-04-16 엔트리 참조.

- [x] 반응형 레이아웃 점검 — MainTable에 `max-h-[calc(100vh-220px)]` 추가 (가로 스크롤바가 뷰포트 내에 상시 노출). ConferenceFormModal에 `mx-4` 추가 (좁은 화면 edge-to-edge 방지)
- [x] 전체적인 디자인 일관성 확인 — 전체 검증 버튼 `indigo-600` → `blue-600` 통일. 나머지 색상/버튼/모달/카드는 baseline에서 이미 일관
- [x] `docs/DESIGN.md` 작성 — 디자인 결정 로그 + baseline 스냅샷
- [ ] 사용자 브라우저 실측 (창 크기 축소, 모달 확인) — 수정 반영 후 확인 필요
- **확인**: 브라우저 창 크기를 줄여도 테이블이 깨지지 않는가? — 실측 대기
