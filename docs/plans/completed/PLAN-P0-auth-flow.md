...
> - `PLAN-P0-quota-policy` §4.7 role 기본값 (v1.1: 신규 가입자 = `user`, 쿼터 10/3)
...
**결정**: PostgreSQL **Trigger** 로 `auth.users` INSERT 시 `public.users` 자동 삽입. 기본 role = `user` (v1.1 정합 — editor 역할 제거, schema §4.1.1 · quota §4.7 과 일치).
...
begin
  ...
  insert into public.quotas (user_id, period_start, update_used, update_limit,
...
**스키마 정합 주의**: `quotas.period_start` 는 `date` 컬럼 (schema §4.1.6 lazy reset). `YYYY-MM` 문자열이 아닌 월 첫날 날짜로 저장. 월초 리셋은 Edge Function 원자적 UPDATE 시 `period_start < date_trunc('month', now() at KST)` 를 만족할 때 0 으로 초기화.

**Role 승격**: user → admin 은 **DB 직접 UPDATE** (파일럿은 운영자 본인 1명만 admin). Phase B 에서 관리자 대시보드 도입 시 자동화. editor 역할은 v1.1 에서 폐기 — PLAN-P0-longterm-vision 의 3채널 기여 모델로 대체 (모든 user 가 자동 검색·수동 입력 가능, 쿼터로만 통제).
...
- **2026-04-22 (v1.1 정합)**: PLAN-P0-longterm-vision (3채널 기여 모델) 확정에 맞춰 §4.8 Trigger SQL 재작성 — (a) 기본 role `viewer` → `user` (editor 역할 폐기), (b) `quotas.month_key` 문자열 삽입 → schema §4.1.6 `period_start` 날짜 컬럼 정합, (c) `on conflict (user_id, month_key)` → `on conflict (user_id)`. Role 승격 문구도 viewer/editor/admin 3단계 → user/admin 2단계로 축소. 의존 블록(§0)에 "신규 가입자 = `user`, 쿼터 10/3" 명시.
