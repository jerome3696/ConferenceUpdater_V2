...
- 30명 파일럿은 "나 + 2~3 editor + viewer 다수" 구조. admin 쿼터 무제한은 개발·QA 실효성 위해 필요하되 전체 예산 공유.
...
- **단계별 처리**:
  ...
  - $45 ≤ 누적 < $50 (90~100%): 모든 viewer 에게 "이번 달 남은 예산 $X — 리셋까지 절약 권장" 배너
  - 누적 ≥ $50: **viewer·editor 호출 차단** (개인 쿼터 잔여와 무관). admin 은 계속 허용 (디버깅 목적).
...
|---|:-:|:-:|---|
| `viewer` | 10 | 3 | roadmap §2 기본값. 30명 × $1.02 = $30.6/월 (< $50) |
| `editor` | 30 | 5 | 공용값 편집자. 3명 가정 시 +$7 |
| `admin` | 999 (실질 무제한) | 999 | 본인. 단 **$50 전체 예산에 본인 비용도 포함** |

계산 근거 (현 v1_1 실측):
- update 1회 ≈ $0.0564
- discovery (Stage1+2) 1회 ≈ $0.15
- viewer 100% 소진: 10×0.0564 + 3×0.15 = $1.014
- editor 100% 소진: 30×0.0564 + 5×0.15 = $2.442
- 30 viewer + 3 editor 전원 100% (극단): $30.4 + $7.3 = **$37.7/월** → $50 상한 내 안전마진 24%
...
- [ ] §4.7 기본값 총합이 월 $50 상한을 **editor 3명 + viewer 30명 100% 소진** 시나리오에서도 넘지 않음 (현재 $37.7)
