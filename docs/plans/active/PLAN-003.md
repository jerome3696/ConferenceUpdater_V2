...
- [x] Step 8 — Group 6: Items 8, 9, 12-css (CSS — word-break, line-clamp)
- [x] Step 9 — Group 6: Item 14 (출처 셀 신뢰도 inline)
- [x] Step 10 — Group 6: Items 6, 7 (액션 버튼 첫 열 + LAST/메모 접기) + MainTable.test.jsx 갱신
- [x] Step 11 — `bash scripts/verify-task.sh` 통과
- [x] Step 12 — qa-backlog.md 14건 [x] 체크 + Completed 섹션 이동
...
- 2026-04-17: Step 8·9·10·11 완료 — Group 6 일괄. (1) `index.css`에 `.cell-text { word-break:keep-all; overflow-wrap:anywhere }`, `.line-clamp-5` 추가 (Items 8, 12-css). (2) 학회명·메모 셀에 `line-clamp-5` + `title` 호버, max-w-xs (Item 9). (3) `formatSourceLabel(source, confidence)`: ai_search일 때 confidence 매핑(고/중/저, high/medium/low) → "AI검색 (X)" 형태, `applyAiUpdate`가 edition에 `confidence` 보존 (Item 14). (4) `ACTION_GROUP`을 `[ACTION_GROUP, ...GROUPS]`로 옮겨 admin일 때 첫 열 (Item 6). (5) Last/참고 그룹에 `collapsible: true`, `collapsed` Set state로 토글, 접힘 시 단일 ▶ 컬럼·열로 축소 (Item 7). MainTable.test.jsx에 레이아웃 케이스 3건 추가. 87 → 90건. verify-task 5/5.
