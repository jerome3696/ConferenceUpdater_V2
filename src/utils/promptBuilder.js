...
// ──────────────────────────────────────────────────────────────────
// Update v1_1 — v1_0 1차 측정 (2026-04-21, pass 16/27) 후 개선.
// 변경 4건:
//   (a) 축 A ①: URL 패턴 3유형 분기 (연도형/회차번호형/불변형)
//   (b) Confidence 재정의 — 소스 신뢰도 기준 (공식 / 준공식 / 외부)
//   (c) 주기 초반 부정확 소스 허용 (null 대신 low 로 채움 → 조기 탐지)
//   (d) iifiir.org 이벤트 슬러그 주기-임박 시 제외 (회차 전용 도메인 강제 탐색)
// 튜닝 상수는 아래 V1_1_VARS — 사용자가 상단만 봐도 조정 가능.
// ──────────────────────────────────────────────────────────────────

// v1_1 튜닝 상수. 프롬프트 본문에 주입되며, 여기 값만 바꿔도 동작 변경.
const V1_1_VARS = {
  // 주기 진행률 = (today - last_end) / (cycle_years × 365 일)
  //   진행률 < 이 값: iifiir.org 이벤트 슬러그 fallback 허용 (초반)
  //   진행률 ≥ 이 값: 회차 전용 독립 도메인 탐색 강제 (임박)
  IIFIIR_STRICT_FROM: 0.7,

  // 부정확 소스 (연구실 페이지·뉴스 단신) 허용 임계값.
  //   진행률 < 이 값: null 대신 low 로 채움 허용 (조기 탐지)
  //   진행률 ≥ 이 값: 공식 소스만 신뢰
  EARLY_INACCURATE_UNTIL: 0.5,
};

const UPDATE_SYSTEM_V1_1 = `당신은 학회/박람회 정보를 검색하는 전문 리서처입니다.
...
function buildUpdateUserV1_1(conference, lastEdition) {
  const today = new Date().toISOString().slice(0, 10);
  const {
    full_name = '',
    abbreviation = '',
