...
- Phase 3 완료 (Step 3.6 실전 검증은 API rate limit으로 MVP 후 QA 이월) / 다음: Phase 4 Step 4.1 (실전 검증 결과 반영 — 이월된 Step 3.6 항목과 연동)
- API rate limit 긴급 개선 진행 중 — `docs/RATE_LIMIT_STRATEGY.md` Phase A 적용 (구조적 토큰 절감). 검증 후 §4.5에 실측 기록, B/C 착수 여부 판단.
- 프롬프트 품질 최적화(v2 가설, subagent 검토)는 `docs/PROMPT_STRATEGY.md` 참조 (rate limit과는 별개 관심사).
- 알려진 이슈:
  - **Anthropic API Tier 1 rate limit**: `web_search` 호출 시 `retry-after` 400초 대기 빈발. 대응은 `docs/RATE_LIMIT_STRATEGY.md` 로드맵(A→B→C)에 따라 단계적 진행.
  - Step 3.6 실전 검증 미수행 → AI 결과 품질에 대한 체계적 검증 부재. Phase A 검증과 함께 재개 예정.
